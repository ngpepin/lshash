...
__device__ void Chunk::g_compress_chunk(u32 out_flags) {
    ...
    for (u32 i = 0; i < leaf_len; i += BLOCK_LEN) {
        ...
        /*
            ...
            but block_cast was never used. The real conversion below reads
            from leaf_data+i, same as the original logic.
        */
...
__global__ void compress_leaves_kernel(Chunk *data, int N) {
    ...
    data[tid].g_compress_chunk();
}

__global__ void compress_parent_level_kernel(Chunk *data, int span, int parent_count) {
    int tid = blockDim.x * blockIdx.x + threadIdx.x;
    ...
    /*
        ...
        The final root ends up in data[0].
    */
    int l = tid * span * 2;
    int right = l + span;
    ...
    /*
        In this project Chunk::data appears to be a u32 array.

        Therefore:
            data[l].data + 8

        means:
            8 * sizeof(u32) = 32 bytes

        That places the right child hash immediately after the left child hash.
    */
    memcpy(data[l].data,     data[l].raw_hash,      32);
    memcpy(data[l].data + 8, data[right].raw_hash,  32);
...
void light_hash(Chunk *data, int N, Chunk *result, Chunk *memory_bar) {
    if (N <= 0) {
        fprintf(stderr, "light_hash: N must be positive, got %d\n", N);
        std::exit(1);
    }

    /*
        This preserves the original assumption in compute():

            // n is always a power of 2

        If your Hasher::_new(file_size) pads N to a power of 2, this is fine.
        If not, this check prevents silent wrong hashes.
    */
    if ((N & (N - 1)) != 0) {
        fprintf(stderr, "light_hash: N must be a power of 2, got %d\n", N);
        std::exit(1);
    }
    ...
    CUDA_CHECK(cudaMemcpy(g_data, data, data_size, cudaMemcpyHostToDevice));

    const int threads = 256;

    // 1. Compress all leaf chunks.
    int leaf_blocks = (N + threads - 1) / threads;

    compress_leaves_kernel<<<leaf_blocks, threads>>>(g_data, N);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());

    // 2. Compress parent levels until data[0] becomes the root.
    for (int span = 1; span < N; span *= 2) {
        int parent_count = N / (2 * span);
        int blocks = (parent_count + threads - 1) / threads;

        compress_parent_level_kernel<<<blocks, threads>>>(g_data, span, parent_count);
        CUDA_CHECK(cudaGetLastError());
        CUDA_CHECK(cudaDeviceSynchronize());
    }

    CUDA_CHECK(cudaMemcpy(result, g_data, sizeof(Chunk), cudaMemcpyDeviceToHost));
}
